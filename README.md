...
**Q1. "ANTHROPIC\_API\_KEY environment variable not set" 오류**

  * 현재 시스템은 OpenAI 모델만 사용하도록 변경되었습니다. `.env` 파일에 `OPENAI_API_KEY`가 있는지 확인하고, 코드 내 불필요한 Anthropic 참조가 있다면 무시해도 됩니다.
...
├── src/
│   ├── shared/
│   │   ├── memory_layer.py      # H-AGM 메모리 코어 (Neo4j/Mem0)
